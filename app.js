...
function refreshWeather() {
	let request = {
		...
		port: 443,
		path:
			'/v1/forecast?latitude=50.85&longitude=4.35&hourly=temperature_2m,precipitation_probability,wind_speed_10m&forecast_days=1',
	};
	...
	function receiveResponseCallback(response) {
		...
		response.on('end', function (chunk) {
			...
			for (let i = 0; i < weatherJSON.hourly.time.length; i++) {
				...
				let date = new Date(weatherJSON.hourly.time[i]).toLocaleDateString('fr-BE', {
					day: 'numeric',
					month: 'long',
					year: 'numeric',
				});
	...
	function receiveResponseCallback(response) {
		...
		response.on('end', function (chunk) {
			...
			filestream.writeFileSync(
				'./cache/rssLiveboard.json',
				JSON.stringify(newLiveboardJSON, null, '\t')
			);
		...
		response.on('end', function (chunk) {
			...
			parseString(rawData, function (err, result) {
				for (let i = 0; i < 2; i++) {
					...
					let gooddate = new Date(date).toLocaleString('fr-FR', {
						day: 'numeric',
						month: 'long',
						year: 'numeric',
					});
...
function refreshFloatrates() {
	...
	function receiveResponseCallback(response) {
		...
		response.on('end', function (chunk) {
			...
			let filteredArrayFloatrates = newFloatRates.floatRatesArray.filter(
				(item) =>
					item.Name === 'U.S. Dollar' ||
					item.Name === 'U.K. Pound Sterling' ||
					item.Name === 'Chinese Yuan'
			);

			filestream.writeFileSync(
				'./cache/rssFloatrates.json',
				JSON.stringify(filteredArrayFloatrates, null, '\t')
			);
...
function refreshNatGeo() {
	...
	function receiveResponseCallback(response) {
		...
		response.on('end', function (chunk) {
			...
			filestream.writeFileSync(
				'./cache/rssNatGeoSpace.json',
				JSON.stringify(rssNatGeoSpaceJSON, null, '\t')
			);
			filestream.writeFileSync(
				'./cache/rssNatGeoTravel.json',
				JSON.stringify(rssNatGeoTravelJSON, null, '\t')
			);
			filestream.writeFileSync(
				'./cache/rssNatGeoAnimals.json',
				JSON.stringify(rssNatGeoAnimalsJSON, null, '\t')
			);
